...
class C_ReprintTicket extends CAaskController {
    ...
    public function execute() {
        ...
        try {
            ...
            switch ($s[0]) {
                case 0:
                    $result = $this->adminDB[$_SESSION["db_1"]]->query($this->select("entry", $_SESSION["db_1"]) . $this->where(array("id" => $s[1], "own" => $_REQUEST["udi"]), "AND"));
                    if ($row = $result->fetch_assoc()) {
                        echo "<strong>";
                        echo "<span style='font-size:30px;'><strong>Lotto</strong></span><br>";
                        ...
                        echo "<span style='font-size:10px;'>Draw ID: " . $row["gametimeid"] . "</span><br>";
                        echo "<span style='font-size:10px;'>Draw Time: " . $row["gameendtime"] . "</span><br>";
                        $this->pritnNumberData($row["point"]);
                        echo"<br>";
                        echo "Invoice: 0-" . $s[1] . "<br>";
