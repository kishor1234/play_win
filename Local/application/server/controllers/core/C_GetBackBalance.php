...
class C_GetBackBalance extends CAaskController {
    ...
    public function execute() {
        ...
        try {
            ...
            $error = array();
            $getBalance=$this->getData($this->select("enduser", $_SESSION["db_1"]).$this->whereSingle(array("userid"=>$this->filterPost("userid"))),"balance");
            if($getBalance<$this->filterPost("balance")){echo $_SESSION["msg"]=$this->printMessage("Insufficent Balance to get back...","danger");die;}
            ...
            $this->adminDB[$_SESSION["db_1"]]->query($this->insert("transaction", $_SESSION["db_1"], array("userid"=>$this->filterPost("userid"),"debit"=>$this->filterPost("balance"),"remark"=>"Admin Get Back Balance from user","ip"=>$_SERVER["REMOTE_ADDR"],"balance"=>$this->getData($this->select("enduser", $_SESSION["db_1"]).$this->whereSingle(array("userid"=>$this->filterPost("userid"))), "balance"))))!=1? array_push($error, "Error on Transcation Table Error Code 02 enduser transaction"):true;
            $this->adminDB[$_SESSION["db_1"]]->query($this->insert("transaction", $_SESSION["db_1"], array("userid"=>$_SESSION["id"],"credit"=>$this->filterPost("balance"),"remark"=>"Self Credit form admin to transfer balance","ip"=>$_SERVER["REMOTE_ADDR"],"balance"=>$this->getData($this->select("user", $_SESSION["db_1"]).$this->whereSingle(array("id"=>$_SESSION["id"])), "balance"))))!=1? array_push($error, "Error on Transcation Table Error Code 02 "):true;
            if (empty($error)) {
